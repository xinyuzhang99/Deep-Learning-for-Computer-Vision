...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        # Loop over all dimensions to fill each value
        for n in range(N):                # for each batch                 
          for f in range(F):              # for each filter                
            for h_out in range(H_out):        
              for w_out in range(W_out):
                chunk = x_pad[n, :, h_out * stride:h_out * stride + HH, w_out * stride:w_out * stride + WW]
                out[n, f, h_out, w_out] = torch.sum(chunk * w[f]) + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        for w_out in range(W_out):
          for h_out in range(H_out):
            for f in range(F):
              for n in range(N):
                dw[f] += dout[n, f, h_out, w_out] * x_pad[n, :, h_out * stride:h_out * stride + HH, w_out * stride:w_out * stride + WW]
                db[f] += dout[n, f, h_out, w_out]
                dx_pad[n, :, h_out * stride:h_out * stride + HH, w_out * stride:w_out * stride + WW] += dout[n, f, h_out, w_out] * w[f,:,:,:]
    ...
    def forward(x, pool_param):
        ...
        for h_out in range(H_out):
          for w_out in range(W_out):
            for c in range(C):
              for n in range(N):
                out[n, c, h_out, w_out] = torch.max(x[n, c, h_out * stride:h_out * stride + pool_height, w_out * stride:w_out * stride + pool_width])
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for h_out in range(H_out):
          for w_out in range(W_out):
            for c in range(C):
              for n in range(N):
                mask = torch.zeros_like(x[n, c, h_out * stride:h_out * stride + pool_height, w_out * stride:w_out * stride + pool_width])
                mask.flatten()[torch.argmax(x[n, c, h_out * stride:h_out * stride + pool_height, w_out * stride:w_out * stride + pool_width])] = dout[n, c, h_out, w_out]
                dx[n, c, h_out * stride:h_out * stride + pool_height, w_out * stride:w_out * stride + pool_width] += mask
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        W2_dim_1 = int(0.5*input_dims[1])
        W2_dim_2 = int(0.5*input_dims[2])
        self.params['W2'] = (torch.randn((num_filters*(W2_dim_1)*(W2_dim_2), hidden_dim), dtype=dtype, device=device)) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        # cache_list = []
        # N = X.shape[0]
        # cache_list.append(Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param))
        # cache_list.append(Linear_ReLU.forward(cache_list[-1][0].reshape(N, -1), W2, b2))
        # cache_list.append(Linear.forward(cache_list[-1][0], W3, b3))
        # scores = cache_list[-1][0]
        ...
        loss += self.reg * torch.sum(self.params['W3'] * self.params['W3'])
        grads_3, grads['W3'], grads['b3'] = Linear.backward(dout, cache_L)
        grads['W3'] += self.reg*2*self.params['W3']
        
        loss += self.reg*torch.sum(self.params['W2'] * self.params['W2'])
        grads_2, grads['W2'], grads['b2'] = Linear_ReLU.backward(grads_3, cache_LR)
        grads['W2'] += 2 * self.reg * self.params['W2']

        loss += self.reg*torch.sum(self.params['W1'] * self.params['W1'])
        grads_1, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(grads_2, cache_CRP)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        #####################################################################
        # Replace "pass" statement with your code
        max_pool_len = len(self.max_pools)

        for i in range(self.num_layers):
          if i == 0:
            if weight_scale == 'kaiming':
              self.params['W'+str(i+1)] = kaiming_initializer(input_dims[0], num_filters[i], K=3, device=device, dtype=dtype)
            else:
              self.params['W'+str(i+1)] = weight_scale * (torch.randn((num_filters[i], input_dims[0], 3, 3), dtype=dtype, device=device))
            ...
            power = 0
            for j in range(max_pool_len):
              if self.max_pools[j] < i:
                power += 1
            height_dim =  int(input_dims[1] / (2**(power)))
            width_dim =  int(input_dims[2] / (2**(power)))

            if weight_scale == 'kaiming':
              self.params['W'+str(i+1)] = kaiming_initializer(num_filters[i-1]*height_dim*width_dim, num_classes, device=device, dtype=dtype)
            else:
              self.params['W'+str(i+1)] = (torch.randn((num_filters[i-1]*height_dim*width_dim, num_classes), dtype=dtype, device=device))*weight_scale
            self.params['b'+str(i+1)] = torch.zeros((num_classes),dtype=dtype, device=device)     
          else:
            if weight_scale == 'kaiming':
              self.params['W'+str(i+1)] = kaiming_initializer(num_filters[i-1], num_filters[i], K=3, device=device, dtype=dtype)
            else:
              self.params['W'+str(i+1)] = (torch.randn((num_filters[i], num_filters[i-1], 3, 3), dtype=dtype, device=device))*weight_scale
            self.params['b'+str(i+1)] = torch.zeros((num_filters[i]),dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
          if i == 0:
            conv_linear_out.append(FastConv.forward(X, self.params['W'+str(i+1)], self.params['b'+str(i+1)], conv_param))
            if self.batchnorm:
              bacthnorm_out.append(SpatialBatchNorm.forward(conv_linear_out[-1][0], self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)], self.bn_params[i]))
              relu_out.append(ReLU.forward(bacthnorm_out[-1][0]))
            else:
              relu_out.append(ReLU.forward(conv_linear_out[-1][0]))
            if i in self.max_pools:
              maxpool_out.append(FastMaxPool.forward(relu_out[-1][0], pool_param))
            else:
              maxpool_out.append(0)
          elif i == self.num_layers - 1:
            if i-1 in self.max_pools:
              conv_linear_out.append(Linear.forward(maxpool_out[-1][0].reshape(N, -1), self.params['W'+str(i+1)], self.params['b'+str(i+1)]))
            else:
              conv_linear_out.append(Linear.forward(relu_out[-1][0].reshape(N, -1), self.params['W'+str(i+1)], self.params['b'+str(i+1)]))
          else:
            if i-1 in self.max_pools:
              conv_linear_out.append(FastConv.forward(maxpool_out[-1][0], self.params['W'+str(i+1)], self.params['b'+str(i+1)], conv_param))
            else:
              conv_linear_out.append(FastConv.forward(relu_out[-1][0], self.params['W'+str(i+1)], self.params['b'+str(i+1)], conv_param))
            if self.batchnorm:
              bacthnorm_out.append(SpatialBatchNorm.forward(conv_linear_out[-1][0], self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)], self.bn_params[i]))
              relu_out.append(ReLU.forward(bacthnorm_out[-1][0]))
            else:
        ...
        for i in range(self.num_layers, 0, -1):
          if i == self.num_layers:
            up_grads, grads['W'+str(i)], grads['b'+str(i)] = Linear.backward(dout, conv_linear_out[-1][1])
            loss += self.reg*torch.sum(self.params['W'+str(i)]**2)
            grads['W'+str(i)] += 2*self.reg*self.params['W'+str(i)]
            if i-2 in self.max_pools:
              up_grads = up_grads.reshape(maxpool_out[-1][0].size())
            else:
              up_grads = up_grads.reshape(relu_out[-1][0].size())
          elif i == 1:
            loss += self.reg*torch.sum(self.params['W'+str(i)]**2)
            if i-1 in self.max_pools:
              up_grads = FastMaxPool.backward(up_grads, maxpool_out[0][1])
            up_grads = ReLU.backward(up_grads, relu_out[0][1])
            
            if self.batchnorm:
              up_grads, grads['gamma'+str(i)], grads['beta'+str(i)] = SpatialBatchNorm.backward(up_grads, bacthnorm_out[0][1])
            up_grads, grads['W'+str(i)], grads['b'+str(i)] = FastConv.backward(up_grads, conv_linear_out[0][1])
            grads['W'+str(i)] += 2*self.reg*self.params['W'+str(i)]
          else:
            loss += self.reg*torch.sum(self.params['W'+str(i)]**2)
            if i-1 in self.max_pools:
              up_grads = FastMaxPool.backward(up_grads, maxpool_out[i-1][1])
            up_grads = ReLU.backward(up_grads, relu_out[i-1][1])

            if self.batchnorm:
              up_grads, grads['gamma'+str(i)], grads['beta'+str(i)] = SpatialBatchNorm.backward(up_grads, bacthnorm_out[i-1][1])
            up_grads, grads['W'+str(i)], grads['b'+str(i)] = FastConv.backward(up_grads, conv_linear_out[i-1][1])
            grads['W'+str(i)] += 2*self.reg*self.params['W'+str(i)]
        #############################################################
        #                       END OF YOUR CODE                    #
        #############################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(input_dims=input_dims, num_classes=10,
                        ...
                        max_pools=[1, 3, 5],
                        reg=1e-5, weight_scale=weight_scale, dtype=dtype,
                        device=device)
    solver = Solver(model, data_dict, 
                    num_epochs=128, batch_size=128,
                    update_rule=adam,
                    optim_config={
                    'learning_rate': 5e-4,
                    },
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.randn((Din, Dout), device=device, dtype=dtype)*(torch.sqrt(torch.tensor(gain/Din)))
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.randn((Dout, Din, K, K), device=device, dtype=dtype)*(torch.torch.sqrt(torch.tensor(gain/(Din*K*K))))
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            sigma = torch.var(x, unbiased=False, dim=0)
            out = (x - mu) / torch.sqrt(sigma + eps)
            out = gamma*out + beta
            running_mean = momentum * running_mean + (1-momentum) * mu 
            running_var = momentum * running_var + (1-momentum) * sigma
            cache = (bn_param, gamma, beta, x, mu, sigma)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
    ...
    def backward(dout, cache):
        ...
        if mode == 'train':
          mu, sigma = cache[4], cache[5]
          x_hat = (x - mu) / torch.sqrt(sigma + eps)
          grad_x_hat = dout * gamma
          grad_sigma = torch.sum((-1/2)*grad_x_hat*(x - mu)*((sigma+eps)**(-3/2)),dim=0)
          grad_mu = torch.sum(grad_x_hat*(-1/torch.sqrt(sigma+eps)), dim=0) + grad_sigma*(torch.sum(-2*(x-mu),dim=0))/N
          dx= grad_x_hat*(1/torch.sqrt(sigma+eps)) + grad_sigma * 2 *(x-mu) / N + grad_mu / N
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        out, cache = BatchNorm.forward(x_new, gamma, beta, bn_param)
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        dout_ = dout.permute((0,2,3,1)).reshape(N*H*W, C)
