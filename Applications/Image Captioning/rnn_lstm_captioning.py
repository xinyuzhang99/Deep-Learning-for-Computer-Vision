...
def rnn_step_backward(dnext_h, cache):
    ...
    dh_before = (1.0 - next_h ** 2) * dnext_h

    dx = torch.matmul(dh_before, Wx.t())
    dprev_h = torch.matmul(dh_before, Wh.t())
    dWx = torch.matmul(x.t(), dh_before)
    dWh = torch.matmul(prev_h.t(), dh_before)
    db = torch.sum(dh_before, dim=0)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
      h_next, cache_ = rnn_step_forward(x[:, i, :], h_prev, Wx, Wh, b)
      h.append(h_next[:, None])
      h_prev = h_next
...
def rnn_backward(dh, cache):
    ...
    for i in range (T-1, -1, -1):
      if i == 0:
        cache_one_step = (x[:, i, :], h0, Wx, Wh, b, h[:, i, :])
      else:
        cache_one_step = (x[:, i, :], h[:, i-1, :], Wx, Wh, b, h[:, i, :])
      if i == T - 1:
        dnext_h = dh[:, i, :]
      else:
        dnext_h = dh[:, i, :] + dprev_h
      dx[:, i, :], dprev_h, dWx_, dWh_, db_ = rnn_step_backward(dnext_h, cache_one_step)
      dWx += dWx_
      dWh += dWh_
      db += db_
    dh0 = dprev_h
...
class WordEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        N, T = x.shape
        x_one_hot = F.one_hot(x, num_classes=V).to(self.W_embed)
        out = torch.matmul(x_one_hot, self.W_embed)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        return out
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        if self.ignore_index is None:
          self.ignore_index = self._null
        self.image_encoder = ImageEncoder()

        if cell_type == "attn":
          self.image_to_hidden = nn.Conv2d(input_dim, hidden_dim, 1)
        else:
          self.image_to_hidden = nn.Linear(input_dim, hidden_dim)
        self.word_encoder = WordEmbedding(vocab_size, wordvec_dim)
        self.hidden_to_vocab = nn.Linear(hidden_dim, vocab_size)
        if cell_type == "rnn":
          self.temporal_model = RNN(wordvec_dim, hidden_dim)
        elif cell_type == "lstm":
          self.temporal_model = LSTM(wordvec_dim, hidden_dim)
        elif cell_type == "attn":
          self.temporal_model = AttentionLSTM(wordvec_dim, hidden_dim)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "attn":
          h0 = self.image_to_hidden(image_emb)
        else:
          image_emb = image_emb.mean(dim=(2, 3))
          h0 = self.image_to_hidden(image_emb)

        words_emb = self.word_encoder(captions_in)
        output = self.temporal_model(words_emb, h0)
        logit = self.hidden_to_vocab(output)
        loss = temporal_softmax_loss(logit, captions_out, self.ignore_index)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def sample(self, images, max_length=15):
        ...
        init_word_emb = torch.zeros_like(captions[:,0][:, None])
        init_word_emb[:, 0] = self._start
        for i in range(max_length):
          if i == 0:
            word_embedding = self.word_encoder(init_word_emb).squeeze(1)
            if self.cell_type == 'rnn':
              next_h = self.temporal_model.step_forward(word_embedding, h0)
            elif self.cell_type == 'lstm':
              next_h, next_c = self.temporal_model.step_forward(word_embedding, h0, c0)
            elif self.cell_type == 'attn':
              attn, attn_weights = dot_product_attention(h0, A)
              next_h, next_c = self.temporal_model.step_forward(word_embedding, h0, c0, attn)
          else:
            word_ind = word_ind[:, None]
            word_embedding = self.word_encoder(word_ind).squeeze(1)
            if self.cell_type == 'rnn':
              next_h = self.temporal_model.step_forward(word_embedding, next_h)
            elif self.cell_type == 'lstm':
              next_h, next_c = self.temporal_model.step_forward(word_embedding, next_h, next_c)
            elif self.cell_type == 'attn':
              attn, attn_weights = dot_product_attention(next_h, A)
              next_h, next_c = self.temporal_model.step_forward(word_embedding, next_h, next_c, attn)
          word_ind = torch.max(self.hidden_to_vocab(next_h), dim=1).indices
          captions[:, i] = word_ind
          if self.cell_type == 'attn':
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        total_gate = torch.matmul(x, self.Wx) + torch.matmul(prev_h, self.Wh) + self.b
        input_gate = torch.sigmoid(total_gate[:, :H])
        forget_gate = torch.sigmoid(total_gate[:, H:2*H])
        output_gate = torch.sigmoid(total_gate[:, 2*H:3*H])
        gate_gate = torch.tanh(total_gate[:, 3*H:4*H])
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(T):
          if i == 0:
            next_h, next_c = self.step_forward(x[:,i,:], h0, c0)
          else:
            next_h, next_c = self.step_forward(x[:,i,:], next_h, next_c)
          hn.append(next_h[:,None])
...
def dot_product_attention(prev_h, A):
    ...
    N, H = prev_h.shape
    prev_h = prev_h[:, None]
    attn_weights = F.softmax((torch.bmm(prev_h, A).squeeze(1))/math.sqrt(H),dim=1)
    attn = torch.sum(A*attn_weights[:, None], dim=-1)
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        N, H = prev_h.shape
        total_gate = torch.matmul(x, self.Wx) + torch.matmul(prev_h, self.Wh) + torch.matmul(attn, self.Wattn) + self.b
        input_gate = torch.sigmoid(total_gate[:, :H])
        forget_gate = torch.sigmoid(total_gate[:, H:2 * H])
        output_gate = torch.sigmoid(total_gate[:, 2 * H:3 * H])
        gate_gate = torch.tanh(total_gate[:, 3 * H:4 * H])
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for i in range(T):
          if i == 0:
            attn, attn_weights = dot_product_attention(h0, A)
            next_h, next_c = self.step_forward(x[:,i,:], h0, c0, attn)
          else:
            attn, attn_weights = dot_product_attention(next_h, A)
            next_h, next_c = self.step_forward(x[:,i,:], next_h, next_c, attn)
          hn.append(next_h[:,None])
        hn = torch.cat(hn, dim=1)
        ######################################################################
        #                           END OF YOUR CODE                         #
