...
def prepocess_input_sequence(
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    string_after_split = input_str.split(' ')
    for i in range(len(string_after_split)):
      if string_after_split[i] in spc_tokens:
        out.append(token_dict[string_after_split[i]])
      else:
        if len(string_after_split[i]) > 1:
          for j in range(len(string_after_split[i])):
            out.append(token_dict[string_after_split[i][j]])
        else:
          out.append(token_dict[string_after_split[i]])
    ##############################################################################
    #               END OF YOUR CODE                                             #
...
def scaled_dot_product_two_loop_single(
    ...
    Kk, M = key.shape
    for i in range(Kq):
      similarity = torch.zeros(Kk, device=query.device, dtype=query.dtype)
      for j in range(Kk):
        similarity[j] = torch.sum(query[i]*key[j]) / torch.sqrt(torch.tensor(M))
      similarity = F.softmax(similarity, dim=0)
      out.append(torch.matmul(value.t(), similarity[:, None]).t())
...
def scaled_dot_product_two_loop_batch(
    ...
    N, Kk, M = key.shape
    for i in range(K):
      similarity = torch.zeros((N, Kk), device=query.device, dtype=query.dtype)
      for j in range(Kk):
        similarity[:, j] = torch.sum(query[:, i, :]*key[:, j, :], dim=1) / torch.sqrt(torch.tensor(M))
      similarity = F.softmax(similarity, dim=1)
      out.append(torch.bmm(value.permute(0,2,1), similarity[..., None]).permute(0,2,1))
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    similarity = torch.bmm(query, key.permute(0,2,1)) / torch.sqrt(torch.tensor(M))
    if mask is not None:
        ##########################################################################
    ...
    # Replace "pass" statement with your code
    weights_softmax = F.softmax(similarity, dim=2)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        torch.nn.init.xavier_uniform_(self.q.weight, gain=1)

        self.k = nn.Linear(dim_in, dim_q)
        torch.nn.init.xavier_uniform_(self.k.weight, gain=1)

        self.v = nn.Linear(dim_in, dim_v)
        torch.nn.init.xavier_uniform_(self.v.weight, gain=1)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.mlp = nn.Linear(dim_out*num_heads, dim_in)
        torch.nn.init.xavier_uniform_(self.mlp.weight, gain=1)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        y = []
        for single_hd in self.heads:
          y.append(single_hd(query, key, value, mask))
        y = torch.cat(y, dim=2)
        y = self.mlp(y)
        ##########################################################################
        #               END OF YOUR CODE                                         #
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        norm_mean = torch.mean(x, dim=-1)
        norm_var = torch.mean((x - norm_mean[...,None])**2, dim=-1)
        y = self.gamma*(x - norm_mean[..., None]) / ((norm_var[...,None]+ self.epsilon)**(0.5)) + self.beta
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out2 = self.drop(out1)
        out3 = self.ln2(self.ff(out2) + out2)
        y = self.drop(out3)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.shape
    mask = torch.zeros((N, K, K), device=seq.device, dtype=seq.dtype)
    indices = torch.triu_indices(K, K, 1)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = torch.zeros(K, M)
    ind1 = torch.arange(0, M, 2)
    ind2 = torch.arange(1, M, 2)
    for i in range(K):
      a = torch.floor(ind1 / M)
      y[torch.tensor(i), ind1] = torch.sin(i / (10000**a))
      if M % 2 == 0:
        y[torch.tensor(i), ind2] = torch.cos(i / (10000**a))
      else:
        y[torch.tensor(i), ind2] = torch.cos(i / (10000**a)[:-1])
    y = y[None, :]
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
