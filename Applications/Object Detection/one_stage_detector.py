...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            if i == 0:
                stem_cls.append(nn.Conv2d(in_channels, stem_channels[i], 3, 1, 1))
                nn.init.normal_(stem_cls[-1].weight, mean=0, std=0.01)
                nn.init.zeros_(stem_cls[-1].bias)
            else:
                stem_cls.append(nn.Conv2d(stem_channels[i-1], stem_channels[i], 3, 1, 1))
                nn.init.normal_(stem_cls[-1].weight, mean=0, std=0.01)
                nn.init.zeros_(stem_cls[-1].bias)
        ...
        for i in range(num_layers):
            if i == 0:
                stem_box.append(nn.Conv2d(in_channels, stem_channels[i], 3, 1, 1))
                nn.init.normal_(stem_box[-1].weight, mean=0, std=0.01)
                nn.init.zeros_(stem_box[-1].bias)
            else:
                stem_box.append(nn.Conv2d(stem_channels[i-1], stem_channels[i], 3, 1, 1))
                nn.init.normal_(stem_box[-1].weight, mean=0, std=0.01)
                nn.init.zeros_(stem_box[-1].bias)
        ...
        # Replace "pass" statement with your code
        self.pred_cls = nn.Conv2d(stem_channels[-1], num_classes, 3, 1, padding=1)
        nn.init.normal_(self.pred_cls.weight, mean=0, std=0.01)
        nn.init.zeros_(self.pred_cls.bias)
        
        self.pred_box = nn.Conv2d(stem_channels[-1], 4, 3, 1, padding=1)
        nn.init.normal_(self.pred_box.weight, mean=0, std=0.01)
        nn.init.zeros_(self.pred_box.bias)

        self.pred_ctr = nn.Conv2d(stem_channels[-1], 1, 3, 1, padding=1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feature_map in feats_per_fpn_level.items():
            class_logits[level_name] = self.pred_cls(self.stem_cls(feature_map))
            B, C, H, W = class_logits[level_name].shape

            class_logits[level_name] = torch.permute(class_logits[level_name].reshape(B, C, H*W), (0, 2, 1))
            boxreg_deltas[level_name] = self.pred_box(self.stem_box(feature_map))

            B, C, H, W = boxreg_deltas[level_name].shape
            boxreg_deltas[level_name] = torch.permute(boxreg_deltas[level_name].reshape(B, C, H*W), (0, 2, 1))
            centerness_logits[level_name] = self.pred_ctr(self.stem_box(feature_map))
            
            B, C, H, W = centerness_logits[level_name].shape
            centerness_logits[level_name] = torch.permute(centerness_logits[level_name].reshape(B, C, H*W), (0, 2, 1))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    deltas[:, :2] = (locations - gt_boxes[:, :2]) / stride
    deltas[:, 2:4] = (gt_boxes[:, 2:4] - locations) / stride
    deltas[(gt_boxes[:, 0] == -1).nonzero()] = -torch.ones_like(gt_boxes[0, :4])
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = None
    # Replace "pass" statement with your code
    numerator = torch.min(deltas[:, 0:4:2], dim=1).values * torch.min(deltas[:, 1:4:2], dim=1).values
    denominator = torch.max(deltas[:, 0:4:2], dim=1).values * torch.max(deltas[:, 1:4:2], dim=1).values
    centerness = torch.sqrt(numerator / denominator)
    centerness[(deltas[:, 0] == -1).nonzero()] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for level_name, featrues in fpn_features.items():
            shape_per_fpn_level[level_name] = featrues.shape
        ...
        matched_gt_deltas = []
        # Replace "pass" statement with your code
        for matched_gt_box_per_image in matched_gt_boxes:
          matched_gt_deltas.append({})
          for level_name, gt_boxes in matched_gt_box_per_image.items():
            matched_gt_deltas[-1][level_name] = fcos_get_deltas_from_locations(locations_per_fpn_level[level_name], gt_boxes, strides_per_fpn_level[level_name])
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # Replace "pass" statement with your code
        onehot_label = (matched_gt_boxes[:,:,4:] + 1.).long()
        cls_targets = F.one_hot(onehot_label, num_classes=self.num_classes+1)[:,:,0, 1:]
        loss_cls = sigmoid_focal_loss(inputs=pred_cls_logits, targets=cls_targets.float())
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            # Replace "pass" statement with your code
            level_pred_cls = torch.max(level_pred_scores, dim=1)
            level_pred_cls_label = level_pred_cls.indices

            # Step 2:
            # Replace "pass" statement with your code
            level_pred_scores = level_pred_cls.values[level_pred_cls.values > test_score_thresh]
            level_pred_classes = level_pred_cls_label[level_pred_cls.values > test_score_thresh]
            level_deltas = level_deltas[level_pred_cls.values > test_score_thresh]
            level_locations = level_locations[level_pred_cls.values > test_score_thresh]
