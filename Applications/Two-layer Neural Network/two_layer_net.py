...
def nn_forward_backward(
    ...
    loss = (torch.log(torch.sum(torch.exp(scores), dim=1)) - scores[torch.arange(num_train), y]).mean()
    ...
    p = torch.exp(scores) / torch.sum(torch.exp(scores), dim=1, keepdim=True) # softmax function
    grad_s = (p - labels) / num_train       # gradient on scores

    # backpropate the gradient to the parameters
    # (downstream_grad = local_grad * upstream_grad)
    # first backprop into parameters W2 and b2
    # scores = max(0, h1 * W2 + b2)
    grad_h1 = torch.mm(grad_s, W2.t())
    grad_W2 = torch.mm(h1.t(), grad_s)
    grad_W2 += reg * 2 * W2  # (H, C) --> grad_W2 is the same dimension as W2
    grad_b2 = torch.sum(grad_s, dim=0)

    # backprop the ReLU non-linearity 
    grad_h1[h1 <= 0] = 0

    # h1 = max(0, X * W1 + b1)
    grad_W1 = torch.mm(X.t(), grad_h1)
    grad_W1 += reg * 2 * W1  # (D, H)
    grad_b1 = torch.sum(grad_h1, dim=0)

    grads['W1'], grads['W2'] = grad_W1, grad_W2
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        # weight_new = weight - learning_rate * grad
      ...
      for hs in hidden_sizes:
        for reg in regularization_strengths:
          for lrd in learning_rate_decays:
            ...
            print('train with regularization_strength: {}'.format(reg))
            print('train with learning rate decay: {}'.format(lrd))
            print('--------------------------------------------------')
            net = TwoLayerNet(3 * 32 * 32, hs, 10, dtype=X_train.dtype, device=X_train.device)
            stats = net.train(X_train, y_train, X_val, y_val, lr, lrd, reg, 3000, 1000)
