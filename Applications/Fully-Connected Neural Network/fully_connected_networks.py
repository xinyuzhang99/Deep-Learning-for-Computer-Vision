...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        x_new = x.view(x.shape[0], -1)
        dw = torch.mm(x_new.t(), dout)
        db = dout.sum(dim=0)    # b: (M,)
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        reg_func = torch.sum(self.params['W1'] * self.params['W1']) + torch.sum(self.params['W2'] * self.params['W2'])
        loss += self.reg * reg_func

        # S2: calculate gradients
        dx2, dw2, db2 = Linear.backward(dout, cache_L2)
        grads['W2'] = dw2 + 2 * self.reg * self.params['W2']
        grads['b2'] = db2

        dx1, dw1, db1 = Linear_ReLU.backward(dx2, cache_L1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        # weight dimension: [D,H1,H2,...,C]
        w_dim = [input_dim]
        w_dim.extend(hidden_dims) # extend(): insert another list to the current list / insert multiple elements
        w_dim.append(num_classes)

        for i in range(self.num_layers):
          self.params['W' + str(i + 1)] = weight_scale * torch.randn((w_dim[i], w_dim[i + 1]), dtype=dtype, device=device)
          self.params['b' + str(i + 1)] = torch.zeros(w_dim[i + 1], dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        ##################################################################
        # Replace "pass" statement with your code
        scores = X      # initialize scores for ReLU forward pass
        cache_list = []

        # First layers: {linear - relu - [dropout]} x (L - 1)}
        for i in range(self.num_layers - 1):
          scores, cache = Linear_ReLU.forward(scores, self.params['W' + str(i + 1)], self.params['b' + str(i + 1)])
          cache_list.append(cache)

          # After each ReLU layer, add a dropout layer
          if self.use_dropout:
        ...
        reg_func = 0
        for i in range(self.num_layers):
          reg_func += torch.sum(self.params['W' + str(i + 1)] * self.params['W' + str(i + 1)])
        loss += 0.5 * self.reg * reg_func
        
        # S2: calculate gradients
        # for the last layer
        dx, dw, db = Linear.backward(dout, cache_list.pop())
        ...
        for j in range(self.num_layers - 1):
          if self.use_dropout:
            dx = Dropout.backward(dx, cache_list.pop())
        
          cache = cache_list.pop()
          dx, dw, db = Linear_ReLU.backward(dx, cache)
          grads['W' + str(self.num_layers - 1 - j)] = dw + self.params['W' + str(self.num_layers - 1 - j)] * self.reg
          grads['b' + str(self.num_layers - 1 - j)] = db
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand(*x.shape, dtype=x.dtype, device=x.device) > p) / (1 - p)
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
