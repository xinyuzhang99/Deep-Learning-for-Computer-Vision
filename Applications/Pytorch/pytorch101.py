...
def count_tensor_elements(x: Tensor) -> int:
    ...
    # Replace "pass" statement with your code
    num_elements = 1
    for i in range(x.dim()):      # x.dim() is an integer
      num_elements *= x.shape[i]  # x.shape has x.dim() elements
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # build a list and then convert it to tensor
    a = torch.arange(start, stop + 1)     # a = [start, stop] (1-D tensor)
    b = []
    for i in range(a.shape[0]):
      if a[i].item() % 10 == 0:
        b.append(a[i])
    x = torch.tensor(b, dtype = torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    last_row = x[-1, :]    # rank = 1 (integer)
    third_col = x[:, 2:3]  # rank = 2 (length)
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idx0 = [1, 0, 3]   # row index
    idx1 = [0, 1, 2]   # column index
    # idx1 = torch.arange(3)
    y = x[idx0, idx1]
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # create an zero tensor with the according size and data type
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # y is composed of two 3 * 4 matrix --> convert x to size (2, 3, 4)
    x1 = x.view(2, 3, -1)   # x1.shape: torch.Size([2, 3, 4])
    y = x1.transpose(0, 1).reshape(3, 8)   # y.shape: torch.Size([3, 2 * 4])
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    _, idx1 = y.min(dim=1)    # find the minimum value of each row
    y[idx0, idx1] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mu = x.sum(dim=0) / x.shape[0]   # mu is the mean of the column
    x_minus_mu = (x - mu)**2
    sigma = torch.sqrt(x_minus_mu.sum(dim=0) * (1 / (x.shape[0] - 1)))
    y = (x - mu)/sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
